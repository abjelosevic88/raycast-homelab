...
function ItemTile(props: { item: AbsItem }) {
  const i = props.item;
  const pct = i.progress !== undefined ? ` · ${Math.round(i.progress * 100)}%` : "";
  return (
    <Grid.Item
...
export default function Audiobooks() {
  ...
  const libraries = useCachedPromise(async (ok: boolean) => (ok ? await listLibraries() : []), [hasToken]);
  const activeLibrary = libraryId || libraries.data?.[0]?.id || "";

  const inProgress = useCachedPromise(async (ok: boolean) => (ok ? await continueListening() : []), [hasToken]);

  const itemsResult = useCachedPromise(
    (lib: string, q: string, ok: boolean) => async ({ page }: { page: number }) => {
      if (!ok || !lib) return { data: [] as AbsItem[], hasMore: false };
      if (q.trim().length >= 2) return { data: page === 0 ? await searchLibrary(lib, q.trim()) : [], hasMore: false };
  ...
  const items = (itemsResult.data ?? []).filter((i) => !seen.has(i.id) && (seen.add(i.id) || true));

  return (
    <Grid
      isLoading={libraries.isLoading || itemsResult.isLoading || inProgress.isLoading}
      ...
      searchBarAccessory={
        <Grid.Dropdown tooltip="Library" value={activeLibrary} onChange={setLibraryId}>
    ...
    >
      {!hasToken && (
        <Grid.EmptyView
          ...
          description="⌘K → Configure Extension → paste your ABS API token (Settings → Users → your user)"
        />
      )}
      {!searching && (inProgress.data?.length ?? 0) > 0 && (
        <Grid.Section title="Continue Listening">
          {inProgress.data?.map((i) => <ItemTile key={`cl-${i.id}`} item={i} />)}
        </Grid.Section>
      )}
