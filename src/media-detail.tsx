...
export default function MediaDetail(props: {
  ...
  const target = data
    ? { mediaType: data.mediaType, id: data.id, title: data.title }
    : undefined;
  const onDone = () => {
    revalidate();
    props.onRequested?.();
  ...
  const markdown = data
    ? [
        ...
        data.posterPath
          ? `<img src="https://image.tmdb.org/t/p/w342${data.posterPath}" height="280" />`
          : "",
        ...
        data && (
          <Detail.Metadata>
            ...
            <Detail.Metadata.Label
              title="Type"
              text={data.mediaType === "tv" ? "Series" : "Movie"}
            />
            {data.rating !== undefined && data.rating > 0 && (
              <Detail.Metadata.Label
                title="Rating"
                text={`★ ${data.rating.toFixed(1)} / 10`}
              />
            )}
            ...
            {data.mediaType === "tv" && data.seasons.length > 0 && (
              <Detail.Metadata.Label
                title="Seasons"
                text={String(data.seasons.length)}
              />
            )}
            {data.network && (
              <Detail.Metadata.Label
                title={data.mediaType === "tv" ? "Network" : "Studio"}
                text={data.network}
              />
            )}
            ...
            {data.cast.length > 0 && (
              <Detail.Metadata.Label title="Cast" text={data.cast.join(", ")} />
            )}
        ...
        target && (
          <ActionPanel>
            {requestable && (
              <Action
                title={
                  target.mediaType === "tv" ? "Request All Seasons" : "Request"
                }
                icon={Icon.Download}
