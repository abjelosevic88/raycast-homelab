...
function BookTile(props: { book: Book }) {
  ...
  const kindleFormats = [...b.formats].sort(
    (x, y) =>
      (KINDLE_ORDER.indexOf(x.format) + 1 || 99) -
      (KINDLE_ORDER.indexOf(y.format) + 1 || 99),
  );
  ...
  async function kindle(f: BookFormat) {
    const toast = await showToast({
      ...
      title: `Sending ${b.title} (${f.format}) to Kindle…`,
    });
    try {
  ...
  async function download(f: BookFormat) {
    const toast = await showToast({
      style: Toast.Style.Animated,
      title: `Downloading ${f.format.toUpperCase()}…`,
    });
    try {
  ...
  return (
    <Grid.Item
      ...
      subtitle={[
        b.authors.join(", "),
        b.year,
        b.formats.map((f) => f.format.toUpperCase()).join("/"),
      ]
        .filter(Boolean)
        .join(" · ")}
      actions={
        <ActionPanel>
          ...
          {kindleFormats.length > 1 && (
            ...
            >
              {kindleFormats.map((f) => (
                <Action
                  key={f.format}
                  title={`${f.format.toUpperCase()} ${fmtSize(f.size)}`}
                  onAction={() => kindle(f)}
                />
              ))}
            </ActionPanel.Submenu>
          ...
          <Action.CopyToClipboard
            title="Copy Author – Title"
            content={`${b.authors.join(", ")} - ${b.title}`}
    ...
    >
      {!hasCreds && (
        <Grid.EmptyView
          ...
          description="⌘K → Configure Extension → set the Calibre-Web URL, user and password"
        />
      )}
      ...
      <Grid.Section
        title={searching ? `Results (${data?.length ?? 0})` : "Recently Added"}
      >
        {data?.map((b) => (
