import { requireUrl, setting } from "./config";
...
export async function loadSubscriptions(): Promise<Subscriptions> {
  const res = await fetch(
    requireUrl("subscriptionsUrl", "Subscriptions JSON"),
...
function fireflyHeaders(): Record<string, string> {
  return {
    Authorization: `Bearer ${setting("fireflyToken")}`,
...
export async function loadMonthTransactions(): Promise<MonthTransaction[]> {
  ...
  for (let page = 1; page <= 5; page++) {
    const res = await fetch(
      `${requireUrl("fireflyUrl", "Firefly III")}/api/v1/transactions?type=withdrawal&start=${from}&end=${to}&limit=100&page=${page}`,
    ...
    for (const t of body.data) {
      for (const s of t.attributes.transactions) {
        out.push({
          ...
          foreign: s.foreign_amount
            ? `${Number(s.foreign_amount).toFixed(2)} ${s.foreign_currency_code}`
            : undefined,
        });
      }
    }
...
export async function loadBills(): Promise<Bill[]> {
  ...
  const res = await fetch(
    `${requireUrl("fireflyUrl", "Firefly III")}/api/v1/bills?start=${from}&end=${to}&limit=100`,
    {
  ...
  return body.data
    .map((b) => ({
      ...
      nextDate: (
        b.attributes.next_expected_match ?? b.attributes.pay_dates?.[0]
      )?.slice(0, 10),
...
export async function loadMonthSpend(): Promise<MonthSpend | undefined> {
  ...
  const to = now.toISOString().slice(0, 10);
  const res = await fetch(
    `${requireUrl("fireflyUrl", "Firefly III")}/api/v1/insight/expense/total?start=${from}&end=${to}`,
    {
  ...
  const row = rows.sort(
    (a, b) => Math.abs(b.difference_float) - Math.abs(a.difference_float),
  )[0];
  if (!row) return { spent: 0, currency: "", from, to };
  return {
    spent: Math.abs(row.difference_float),
